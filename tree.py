...
class Tree:
	def __init__(self, game, num_rollouts, C, max_depth, timeout):
		print("tree init")
		self.game = game
...
class Node:
	...
	def generate_children(node):
		...
		res = []
		free_locns = np.argwhere(node._invboard) # array of nX2
...
def perform_rollouts(board, current_player, last_move_at, tree:Tree):
	...
	inqueue = mp.Queue()
	outqueue = mp.Queue()
	pool = mp.Pool(processes=4, initializer=rollout_workers, initargs=(inqueue, outqueue, tree))
